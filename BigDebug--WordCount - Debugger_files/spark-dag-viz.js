...
/*
 On load function to instantiate the websocket
 */


function initWebSocket(type, rdd) {
    ...
    switch (type) {
        case 1:
            ...
            ws.onmessage = function (e) {
                ...
                if (!server_message.startsWith("E")) {
                    document.getElementById("table_wp").innerHTML = server_message
                }
                ;
            }
            break;
        case 2:
            ws.onmessage = function (e) {
                ...
                if (server_message.startsWith("<")) {
                    document.getElementById("dag-viz-graph").innerHTML = ""
                    document.getElementById("dag-viz-metadata").outerHTML = server_message
                    renderDagViz(true)
                } else if (server_message.startsWith("[")) {
                    updateChart(server_message);
                }
                else if (server_message.startsWith("Error")) {
                    var val = parseInt(server_message.split(" ")[1])
        ...
        case 3 :
            ...
            ws.onmessage = function (e) {
                var server_message = e.data;
                console.log(server_message);
                if (!server_message.startsWith("Est")) {
                    updateChart(server_message);
                }
                ;
            }
...
/** WatchPoint content puller

 function doPoll(time, link){
  $.get(link, function(data) {
      document.getElementById("table_wp").innerHTML=data;  // process results here
...
/*
 ...
 Render Dynamic Worker DataProfile Charts
 Using CanvasJs

 */
var chart = null
var dps = null;
function chartRender(rdd, wid) {
    dps = [{
        label: "",
        y: 0,
        link: ""
    ...
    if (rdd !== undefined)
        var title = "Latency Monitoring at Worker[" + wid + "] on RDD[" + rdd + "]\n"
...
function updateChart(data) {
    ...
    for (var i = 0; i < data_object.length; i++) {
        if (data_object[i].stddev !== undefined) {
            std = data_object[i].stddev
        } else {
            if (data_object[i].y < min) {
                min = data_object[i].y
            }
            if (data_object[i].y > max) {
                max = data_object[i].y
            }
            if (data_object[i].label === "Average")
                avg = data_object[i].y
        }
    ...
    for (var i = 0; i < data_object.length; i++) {
        if (data_object[i].stddev === undefined) {
            var yVal = data_object[i].y;
            var R = Math.round((255 * (yVal - min)) / (max - min));
            var G = Math.round((255 * ((max - min) - (yVal - min))) / (max - min));
            var B = 0;
            data_object[i].label = (data_object[i].label.length > 20) ? data_object[i].label.substring(0, 20) : data_object[i].label;
            dps[i] = {
                label: data_object[i].label,
                y: yVal,
                link: data_object[i].link,
...
function createCode() {
    ...
    var lineNum, percent, totalNum;
    var sum = editor.lineCount();
    var color = d3.scaleLinear().domain([1,sum])
                .range(["palegreen","pink"]);
    d3.csv("data.csv", function(error, csvdata){
        ...
        for(var i = 0;i<csvdata.length;i++){
            lineNum = csvdata[i].line;
            percent = csvdata[i].percent;
            editor.addLineClass(parseInt(lineNum), "background", "styled-background"+lineNum);
            d3.select(".styled-background"+lineNum).style("background",color(percent*sum));
        }   
    });
};
...
/*
function renderPie() {
    var pieData = d3.select("#col3").select("#test-data").text();
    var g = Dot.read(pieData);
    var renderer = new dagreD3.render();
    var container = d3.select("#col3").select("#graph");
    renderer(container, g);
...
function addTooltipsForRDDs(svgContainer) {
    svgContainer.selectAll("g.node").each(function () {
        ...
        node.on("mouseenter", function () {
            triggerTooltipForRDD(node, true);
        });
        node.on("mouseleave", function () {
            triggerTooltipForRDD(node, false);
        });
    });
